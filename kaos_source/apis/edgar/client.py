"""SEC EDGAR HTTP client — raw async functions.

No API key required, but the SEC mandates a descriptive ``User-Agent``
header on all requests (format: ``"YourCompany contact@company.com"``).
Rate limit: 10 requests/second.

This module is the implementation layer for the
:class:`EdgarApiConnector` and the EDGAR MCP tools: bytes in, structured
records out.
...
Configuration via :class:`KaosSourceEdgarSettings` (env prefix
``KAOS_SOURCE_EDGAR_``):

- ``KAOS_SOURCE_EDGAR_USER_AGENT`` — Required User-Agent (legacy
  ``SEC_EDGAR_USER_AGENT`` honored)
