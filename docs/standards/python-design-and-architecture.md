...
These standards apply to Python code in `kaos-source`.

`kaos-source` publishes the `kaos_source` import package. This package publishes CLI entry point(s): `kaos-source`, `kaos-source-serve`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-source` publishes import package `kaos_source`.
- Declare the public API in `kaos_source.__all__` when the package has
...
- Names exported from `kaos_source.__all__`.
...
- Put optional integrations in named extras. Public extras currently declared: `browser`, `content`, `pacer`.
