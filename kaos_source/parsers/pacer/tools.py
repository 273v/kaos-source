        ...
        return ToolMetadata(
            ...
            input_schema=[
                ParameterSchema(
                    ...
                    type="string",
                    description=(
                        "Path to the saved PACER docket HTML file. Accepts an "
                        "absolute filesystem path, a kaos://artifacts/<id> URI, or "
                        "a relative path / kaos:// URI that resolves inside the "
                        "session VFS."
                    ),
                ),
            ],
        ...
        try:
            async with resolve_source_input(path_str, context) as resolved:
                ...
                output = {
                    ...
                    "date_terminated": str(docket.date_terminated)
                ...
                summary = (
                    f"{docket.case_number}: {docket.case_title} — {docket.entry_count} entries"
                )
                if docket.warnings:
