"""SourceService — routes :class:`SourceLocator` to the right connector and runs jobs.

Moved from ``kaos_source/service.py`` (which is now a back-compat shim
that re-exports this class). Behavior unchanged — this is a chunk 2
refactor that consolidates execution machinery into ``runtime/``.

Chunk 4 will rewire ``SourceService.__init__`` to consume connectors
from :class:`ConnectorRegistry` instead of hardcoding the 5 transport
connectors. For chunk 2 the constructor still hardcodes them — the
move is structural only.
"""
