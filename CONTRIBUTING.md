...
`kaos-source` requires Python 3.13 or newer. It publishes the `kaos_source`
import package. This package publishes CLI entry point(s): `kaos-source`, `kaos-source-serve`.
Public extras currently declared: `browser`, `content`, `pacer`.
...
include the `kaos-source` version, Python version, operating system,
